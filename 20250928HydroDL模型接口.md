...
jsonify({'status': 'success', 'msg': f'No mesh data exists. Please reslice the mesh.'}), 200
```
...
> workspace_name  # # 工作方案名
> prj_name # 计算方案名称
> rainfall_data # 降雨时间序列数据 第一列是以秒为单位的时间列，第二例是降雨数据，单位m/h
> ```
...
> http://127.0.0.1:18896/grid/ele_adjustment
...
> workspace_name  # # 工作方案名
> prj_name # 计算方案名称
> ele_vertices_data 修正后的三角网格顶点的地形数组，维度为[N, 3] N为三角网格个数。
> ```

### 成功响应示例

```json
jsonify({'status': 'success', 'msg': f'Terrain interpolation successful', "data": triangles_infos}), 200

{'status': 'success', 'msg': 'domain file missing!'}, 200  # 可能没模型初始化
```

### 失败响应示例

```
jsonify({'status': 'error', 'msg': f'Terrain interpolation status retrieval failed. Please try again: {str(e)}'}), 500
```
...
> http://127.0.0.1:18896/cal/run_hydrodynamic_model
...
```
jsonify({'status': 'error', 'msg': f'结果获取失败，请重试: {str(e)}'}), 500
```
...
> http://127.0.0.1:18896/cal/stop_hydrodynamic_model
...
```
jsonify({'status': 'error', 'msg': f'计算进程终止失败，请重试: {str(e)}'}), 500
```
...
> http://127.0.0.1:18896/cal/hydrodynamic_model_state
...
```json
{'status': 'success', 'start_time': prj_infos["start_time"], "step": prj_infos["step"], "end_time": prj_infos["end_time"]}
{'status': 'calculating', 'start_time': prj_infos["start_time"], "step": prj_infos["step"]}
{'status': 'waiting'}
...
> http://127.0.0.1:18896/result/model_predict
...
```json
{'status': 'success', 'data': result.tolist()}
...
> http://127.0.0.1:18896/result/get_point_wl_process
...
> workspace_name  # # 工作方案名
> prj_name # 计算方案名称
> point_station：表示点的经纬度  [120.2222, 36.0000]
> ```

### 成功响应示例

```json
jsonify({'status': 'success', 'point_wl': wl})
```

### 失败响应示例

```
jsonify({'status': 'end', 'data': [], "trgs": [], "node_xy": []}), 500
```
...
> http://127.0.0.1:18896/prj/create_project
...
```json
jsonify({'status': 'success', 'msg': '计算方案创建成功'})
```

### 失败响应示例

```
jsonify({'status': 'error', 'msg': f'计算方案创建失败，请重试: {str(e)}'})
```
...
> http://127.0.0.1:18896/prj/delete_project
...
```json
jsonify({'status': 'success', 'msg': '计算方案已删除'})
jsonify({'status': 'success', 'msg': '未找到该计算方案，请创建'})
```

### 失败响应示例

```
jsonify({'status': 'error', 'msg': f'计算方案删除失败，请重试: {str(e)}'})
```
...
> http://127.0.0.1:18896/result/get_risk_graph_process
